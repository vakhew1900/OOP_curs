...
public abstract class AbstractPlumber {
     ...
     */
    public AbstractPlumber(GameField gameField) {
    ...
    void createPipeline() {
        ...
        source = createSource(directionList.get(0), cellPath.get(0));

        pipeList = createPipePath(startCell, directionList);
        drain =  createDrain(directionList.get(directionList.size() - 1), cellPath.get(cellPath.size() - 1));

    }

    protected abstract Source createSource(Direction direction, Cell cell);

    protected abstract Drain createDrain(Direction direction, Cell cell);
    ...
    private List createPipePath(@NotNull Cell cell, @NotNull List<Direction> directionList) {
        ...
        for (int i = 1; i < directionList.size(); i++) {
            ...
            directions.add(directionList.get(i - 1));
            directions.add(directionList.get(i));

            Pipe pipe = createPipe(directions, cell, pipeList);
            pipeList.add(pipe);
    ...
    protected abstract  Pipe createPipe(List<Direction> directions, Cell cell, List<Pipe> pipeList);
